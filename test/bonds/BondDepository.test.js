...
describe("Bond Depository", async () => {
    ...
    // Initial mint for Frax, ENCTR and DAI (10,000,000)
    ...
    beforeEach(async () => {
        ...
        encountr = await erc20Factory.deploy("Encountr", "ENCTR", 9);
        treasury = await smock.fake("ITreasury");
        gENCTR = await gEnctrFactory.deploy("50000000000"); // Set index as 50
        staking = await smock.fake("EncountrStaking");
        ...
        await encountr.mint(deployer.address, "10000000000000");
        await treasury.baseSupply.returns(await encountr.totalSupply());

        // Mint enough gENCTR to payout rewards
        await gENCTR.mint(depository.address, "1000000000000000000000");

        await encountr.connect(alice).approve(depository.address, LARGE_APPROVAL);
    ...
    it("should not redeem before vested", async () => {
        let balance = await encountr.balanceOf(bob.address);
        let amount = "10000000000000000000000"; // 10,000
        await depository
            .connect(bob)
            .deposit(bid, amount, initialPrice, bob.address, carol.address);
        await depository.connect(bob).redeemAll(bob.address, true);
        expect(await encountr.balanceOf(bob.address)).to.equal(balance);
    });
    ...
    it("should redeem after vested", async () => {
        ...
        const bobBalance = Number(await gENCTR.balanceOf(bob.address));
        expect(bobBalance).to.greaterThanOrEqual(Number(await gENCTR.balanceTo(expectedPayout)));
        expect(bobBalance).to.lessThan(Number(await gENCTR.balanceTo(expectedPayout * 1.0001)));
    });

    it("should give correct rewards to referrer and dao", async () => {
        let daoBalance = await encountr.balanceOf(deployer.address);
        let refBalance = await encountr.balanceOf(carol.address);
        let amount = "10000000000000000000000"; // 10,000
        ...
        // Mint encountr for depository to payout reward
        await encountr.mint(depository.address, "1000000000000000000000");

        let daoExpected = Number(daoBalance) + Number((Number(payout) * daoReward) / 1e4);
        await depository.getReward();

        const frontendReward = Number(await encountr.balanceOf(deployer.address));
        ...
        const carolReward = Number(await encountr.balanceOf(carol.address));
